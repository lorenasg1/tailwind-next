...
type FileItemProps = ComponentProps<'div'> & VariantProps<typeof fileItem> & {
  name: string
  size: number
}
      ...
      <div className={icon()}>
        ...
        {state === 'error' ? (
          <div className="flex flex-col flex-1 items-start gap-1">
            ...
            <button type="button" className="text-sm font-semibold text-red-700 hover:text-red-900 dark:text-error-400 dark:hover:text-error-300">Try again</button>
          </div>
        ) : (
          <div className="flex flex-col flex-1 items-start gap-1">
            <div className="flex flex-col">
            ...
            <div className="flex w-full items-center gap-3">
              <div className="h-2 flex-1 rounded-full bg-zinc-100 dark:bg-zinc-600">
                <div className="h-2 w-4/5 rounded-full bg-violet-600 dark:bg-violet-400" style={{ width: state === 'complete' ? '100%' : '80%' }} />
              </div>
        ...
        {state === 'complete' ? (
          <CheckCircle2 className="h5- w-5 fill-violet-600 text-white" />
        ) : (
